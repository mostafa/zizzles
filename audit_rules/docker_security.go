...
// Helper methods for determining which checks to run
func (d *DockerSecurityDetector) shouldCheckUnpinned(node ast.Node) bool {
	var nodeValue string
	switch n := node.(type) {
	case *ast.StringNode:
		nodeValue = n.Value
	...
	nodeValue = strings.Trim(nodeValue, "\"'")
	return strings.Contains(nodeValue, "docker://") && strings.Contains(nodeValue, ":")
}

func (d *DockerSecurityDetector) shouldCheckRootUser(node ast.Node) bool {
	var nodeValue string
	switch n := node.(type) {
	case *ast.StringNode:
		nodeValue = n.Value
	...
	nodeValue = strings.Trim(nodeValue, "\"'")
	return strings.Contains(nodeValue, "FROM ") && !strings.Contains(nodeValue, "USER ")
}

func (d *DockerSecurityDetector) shouldCheckSecrets(node ast.Node) bool {
	var nodeValue string
	switch n := node.(type) {
	case *ast.StringNode:
		nodeValue = n.Value
	...
	nodeValue = strings.Trim(nodeValue, "\"'")
	secretPatterns := []string{"echo $INPUT_", "echo ${{ secrets.", "echo ${INPUT_", "print($INPUT_", "console.log($INPUT_", "printf $INPUT_"}
	for _, pattern := range secretPatterns {
		if strings.Contains(nodeValue, pattern) {
			return true
		}
...
func (d *DockerSecurityDetector) shouldCheckDevTools(node ast.Node) bool {
	var nodeValue string
	switch n := node.(type) {
	case *ast.StringNode:
		nodeValue = n.Value
